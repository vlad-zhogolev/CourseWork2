...
void SceneLoader::loadScene(std::string lightsDataPath, std::string modelsDataPath,
...
{
    loadLights(lightsDataPath, dirLights, pointLights, spotLights);
    loadModels(modelsDataPath, models, objects);
}

void SceneLoader::loadLights(const std::string& lightsDataPath, DirLightsSharedPtr dirLights,
...
{    
    try
    {
        ...
        bool good = true; // flag for checking that light attributes satisfy conditions
        int counter = 1;
        string type;
        while (lightData >> type)
        {
            if (type == "point")
...
void SceneLoader::loadModels(const std::string & modelsDataPath, std::vector<Model>& models,
    std::vector<Object>& objects)
{
    ...
    try
    {
        ...
        while (!objectsData.eof())
        {
            ...
            if (!checkRangeVec3(positions[positions.size() - 1], -1000, 1000,
                "ERROR::SCENE_LOADER::WRONG_MODEL_POSITION model number: " + to_string(positions.size())) ||
            ...
            // check if model already loaded (getModelIndex return is > 0)
